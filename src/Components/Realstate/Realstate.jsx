...
const Realstate = () => {
  ...
  async function fetchDataFromFirestoreByCategory() {
    try {
      ...
      const querySnapshot = await getDocs(query(collection(db, 'Artciles'), where('categoryName', '==', 'عقارات')));
  ...
  return (
    <div className=' w-60 bg-white my-3 shadow margin'>
      {loading ? (
        ...
        <>
          <h4 className=' px-2 py-2 shadow brdr-top brdr-bottom fw-bolder'>عقارات</h4>
